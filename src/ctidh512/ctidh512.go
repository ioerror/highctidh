...
void custom_gen_private(void *const context, highctidh_512_private_key *priv) {
  highctidh_512_csidh_private_withrng(priv, (uintptr_t)context, fillrandom_512_custom);
}
*/
import "C"
import (
...
type PublicKey struct {
	publicKey C.highctidh_512_public_key
}
...
func (p *PublicKey) FromBytes(data []byte) error {
	...
	p.publicKey = *((*C.highctidh_512_public_key)(unsafe.Pointer(&data[0])))
	if !C.highctidh_512_validate(&p.publicKey) {
...
type PrivateKey struct {
	privateKey C.highctidh_512_private_key
}
...
func (p *PrivateKey) Bytes() []byte {
	return C.GoBytes(unsafe.Pointer(&p.privateKey), C.sizeof_highctidh_512_private_key)
}
...
func (p *PrivateKey) FromBytes(data []byte) error {
	...
	p.privateKey = *((*C.highctidh_512_private_key)(unsafe.Pointer(&data[0])))
	return nil
}
...
func DerivePublicKey(privKey *PrivateKey) *PublicKey {
	...
	C.memset(unsafe.Pointer(&base), 0, C.sizeof_highctidh_512_public_key)
...
func GenerateKeyPair() (*PrivateKey, *PublicKey) {
	...
	C.highctidh_512_csidh_private(&privKey.privateKey)
...
func GroupAction(privateKey *PrivateKey, publicKey *PublicKey) *PublicKey {
	...
	ok := C.highctidh_512_csidh(&sharedKey.publicKey, &publicKey.publicKey, &privateKey.privateKey)
	if !ok {
		panic(ErrCTIDH)
	}
	return sharedKey
...
func init() {
	...
	PublicKeySize = 64
	PrivateKeySize = C.sizeof_highctidh_512_private_key
}
