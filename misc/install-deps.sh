...
clearlinux)
	swupd info
	INSTALLCMD="swupd bundle-add -y" ;;

debian|devuan|ubuntu|x-ubuntu*)
	export DEBIAN_FRONTEND=noninteractive

	PREPARE="apt update"
	INSTALLCMD="apt install -y --no-install-recommends"
...
x-ubuntu-24.04-cross*)
	...
	ARCH="${CONTAINER#x-ubuntu-24.04-cross,}"
	case "$ARCH" in
	"$CONTAINER"|"") ;;

	386|i386)
		CROSS_PKGS="gcc-i686-linux-gnu libc6-dev-i386-cross" ;;

	amd64*)
		;;

	arm|arm32v*)
		CROSS_PKGS="gcc-arm-linux-gnueabi gcc-arm-linux-gnueabihf libc6-dev-armel-cross libc6-dev-armhf-cross" ;;

	arm64*)
		CROSS_PKGS="gcc-aarch64-linux-gnu libc6-dev-arm64-cross" ;;

	ppc64)
		CROSS_PKGS="gcc-powerpc64-linux-gnu libc6-dev-powerpc-ppc64-cross" ;;

	mipsle|mipsel)
		CROSS_PKGS="gcc-mipsel-linux-gnu libc6-dev-mipsel-cross" ;;

	mips64)
		CROSS_PKGS="gcc-mips64-linux-gnuabi64 libc6-dev-mips64-cross" ;;

	mips64le|mips64el)
		CROSS_PKGS="gcc-mips64el-linux-gnuabi64 libc6-dev-mipsn32-mips64el-cross" ;;

	ppc64le)
		CROSS_PKGS="gcc-powerpc64le-linux-gnu libc6-dev-ppc64el-cross" ;;

	mips|riscv64|s390x)
		CROSS_PKGS="gcc-$ARCH-linux-gnu libc6-dev-$ARCH-cross" ;;

	*)
		CROSS_PKGS="gcc gcc-arm-linux-gnueabi \
	...
	PKGS="$PKGS lld $CROSS_PKGS"
	;;
esac


echo "Installing required packages..."
test "$1" && echo "Installing extra packages: $*"

( $INSTALLCMD $PKGS "$@" || $INSTALLCMD $PKGS "$@" ) > /dev/null
